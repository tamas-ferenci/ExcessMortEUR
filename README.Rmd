...
geodata <- eurostat::get_eurostat_geospatial(output_class = "sf", resolution = "20",
                                             nuts_level = 0, year = "2021")
geodata <- merge(geodata, res[, tail(.SD, 1), .(geo)][, .(y = cumexcess/meanpopulation*1e6, geo)],
                 by = "geo")
```
...
ggplot(geodata, aes(fill = y)) + geom_sf() + coord_sf(xlim = c(-25, 50), ylim = c(33, 73)) +
