...
output:
  ...
  github_document:
    ...
    toc_depth: 3
  # word_document:
  #   pandoc_args: ["--lua-filter=pandoc-quotes.lua", "--lua-filter=link-to-footnote.lua"]
  #   toc: true
  #   toc_depth: 3
...
knitr::opts_chunk$set(echo = knitr::is_html_output(), fig.width = 12, fig.height = 6, dpi = 300,
                      dev = "ragg_png")
```
...
DemData <- rbind(
  cbind(Type = "Esetszám",
        fread(paste0("https://github.com/CSSEGISandData/COVID-19/raw/master/",
                     "csse_covid_19_data/csse_covid_19_time_series/",
                     "time_series_covid19_confirmed_global.csv"), check.names = TRUE)),
  cbind(Type = "Halálozások száma",
        fread(paste0("https://github.com/CSSEGISandData/COVID-19/raw/",
                     "master/csse_covid_19_data/csse_covid_19_time_series/",
                     "time_series_covid19_deaths_global.csv"), check.names = TRUE)))
DemData <- melt(DemData, id.vars = c("Type", "Province.State", "Country.Region", "Lat", "Long"))
DemData$date <- as.Date(substring(DemData$variable, 2), "%m.%d.%y")
DemData[, value := c(0, diff(value)), .(Type, Country.Region)]
DemData$value <- as.double(DemData$value)
DemData[, value := filter(value, rep(1/7, 7), sides = 2), .(Type, Country.Region)]
DemData[Type=="Halálozások száma"]$value <- DemData[Type=="Halálozások száma"]$value * 60

ggplot(DemData[Country.Region=="Czechia"&date>="2020-09-16"&date<="2020-12-16"],
       aes(x = date, y = value, group = Type, color = Type)) + geom_line() +
  scale_y_continuous(sec.axis = sec_axis(~ . / 60)) +
  scale_x_date(sec.axis = sec_axis(~., breaks = seq.Date(as.Date("2020-09-16"),
                                                         as.Date("2020-12-16"), by = "weeks"),
                                   labels = seq.Date(as.Date("2020-09-16"),
                                                     as.Date("2020-12-16"),
                                                     by = "weeks")-as.Date("2020-10-14"))) +
  geom_vline(xintercept = as.Date("2020-10-14")) +
  labs(x = "", y = "", color = "",
       caption = "Ferenci Tamás, https://github.com/tamas-ferenci/ExcessMortEUR/") +
  theme(legend.position = "bottom", axis.text.y.left = element_text(color = "#F8766D"),
...
Nézzük meg például, hogy a [KSH adatai](https://www.ksh.hu/stadat_files/nep/hu/nep0010.html) alapján hogyan alakult 2010 óta az autóbalesetben és az öngyilkosság miatt meghaltak száma Magyarországon:

```{r, echo = FALSE, message = FALSE}
ggplot(melt(data.table(openxlsx::read.xlsx(
  "https://www.ksh.hu/stadat_files/nep/hu/nep0010.xlsx",
  na.strings = "–", startRow = 2), check.names = TRUE)[(which(Év=="Összesen")+1):(.N)][
  ...
  labs(y = "Halálozások száma [fő/év]", color = "",
       caption = paste0("Ferenci Tamás, https://github.com/tamas-ferenci/ExcessMortEUR/\n",
                        "Adatok forrása: KSH")) +
...
Az e kérdés iránt mélyebben érdeklődőeknek figyelmébe ajánlom egy [friss cikkem](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/s12874-023-02061-w), melyben ezt a problémakört jártam körbe részletes vizsgálatokkal.
...
Mostani elemzésemben egy olyan módszert fogok használni, mely a fenti gondolatokon alapszik, azokat valósítja meg, de statisztikai értelemben még szofisztikáltabb. Az eljárást Rolando J. Acosta (Harvard University) és Rafael A. Irizarry (Dana-Farber Cancer Institute) 2020-ban [publikálta](https://journals.lww.com/epidem/abstract/2022/05000/a_flexible_statistical_framework_for_estimating.6.aspx).
...
Visszatértve még egy pillanatra az ábrára: ha ránézünk, az is látszik, miért lehet komoly hibaforrás a járvány végének a rossz meghatározása (ami pedig, mint láttuk, sajnos egyáltalán nem elképzelhetetlen). A 2023-as finn adatok nagyon magasak. De vajon ez azért van, mert a finn adatoknak ez lett volna járvány nélkül is az alaptrendjük, csak kicsit meredekebben nőttek, mint gondoltuk -- nem lehetetlen, tényleg növekedtek már a járvány előtt is, csak kevésbé meredeken -- vagy azért ilyen magas, mert valójában ebben még igenis benne van a járvány hatása, még nem tértünk vissza az alaptrendhez, és ezért emelkedett ki annyira...? (És vigyázat, ahogy volt is róla szóla, ide nem csak a direkt hatásokat kell érteni, hanem az indirekteket is.) Ezért az ex post adatokat is óvatosan kell kezelni.
...
ggplot(res[nuts_level==0&age=="TOTAL"&sens==FALSE&ED=="ExAnte"&model=="quasipoisson"&
             date<="2023-07-01"],
       aes(x = date, y = excess/population*1e6, group = geo)) + geom_line() +
  labs(x = "", y = "Ex ante aktuális\ntöbblethalálozás [fő/1M fő]", caption = captionlab) +
...
ggplot(res[nuts_level==0&age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&
             date=="2023-06-26"][order(cumexcess/meanpopulation)],
       aes(x = factor(geo, levels = geo), y = cumexcess/meanpopulation*1e6, fill = geo=="HU")) +
...
Látható, hogy Magyarország a legkedvezőtlenebb harmad elején-közepén van. Hogy egy számszerű érték is szerepeljen: a kumulált többlethalálozásunk a járvány alatt ex post számításban `r format(round(res[geo=="HU"&age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&
date=="2023-06-26"]$cumexcess, -2), scientific = FALSE)` fő volt.
...
ggplot(geodata[geodata$nuts_level==0&geodata$age=="TOTAL",], aes(fill = y)) + geom_sf() +
  coord_sf(xlim = c(-25, 50), ylim = c(33, 73)) +
  scale_fill_distiller(palette = "Oranges", direction = 1) +
  labs(fill = "Ex post kumulált\ntöbblethalálozás [fő/1M fő]", caption = captionlab) +
  theme(plot.caption = element_text(face = "bold", hjust = 0))
```
...
ggplot(res[age=="TOTAL"&(nuts_level==3|geo=="HU")&sens==FALSE&ED=="ExAnte"&
             model=="quasipoisson"&date<="2023-07-01"],
       aes(x = date, y = excess/population*1e6, group = geoname, label = geoname)) +
  geom_line(aes(color = geo=="HU",
                group = forcats::fct_reorder(geo, geo=="HU", .fun = first))) +
  geom_hline(yintercept = 0, colour = "blue") +
  scale_color_manual(values = c("FALSE" = "gray", "TRUE" = "red")) + guides(color = "none") +
...
ggplot(res[(nuts_level==3|geo=="HU")&age=="TOTAL"&sens==FALSE&ED=="ExPost"&
             model=="quasipoisson"&date=="2023-06-26"][order(cumexcess/meanpopulation)],
       aes(x = factor(geoname, levels = geoname), y = cumexcess/meanpopulation*1e6,
           fill = geo=="HU")) +
  geom_col() + guides(fill = "none") +
  labs(x = "", y = "Ex post kumulált\ntöbblethalálozás [fő/1M fő]", caption = captionlab) +
  theme(plot.caption = element_text(face = "bold", hjust = 0), legend.position = "bottom",
...
A korábban már felvázolt "lassú" adatszolgáltatási folyamatnak, tehát a HVB-k alapján történő, az Egészségügyi Világszervezet protokollját követő besorolásnak az [eredményei megvannak](https://www.ksh.hu/stadat_files/nep/hu/nep0010.html), éves felbontásban: a 2020 évi összesített adat szerint 8981 halálesetet, a 2021-es szerint 24838-at, a 2022-es szerint 7685-öt soroltak a mostani koronavírus miatt bekövetkezettnek. Érdemes -- pontokkal -- ezeket is megjelölni az ábrán:
...
Mint látszik, érdemi különbség 2020-ban nincs, 2021-ben és 2022-ben pedig egy kisebb eltérés alakult ki: a napi gyorsjelentett halálozások száma némileg a halottvizsgálati szám fölé kerül, ám a különbség nem jelentős (2022-ig összesítve a jelentett halálozás `r format(res[geo=="HU"&age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&date=="2022-12-26"]$cumnewdeaths, scientific = FALSE)` fő, a halottvizsgálati érték `r format(8981 + 24838 + 7685, scientific = FALSE)` fő, tehát a különbség `r round((res[geo=="HU"&age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&date=="2022-12-26"]$cumnewdeaths/(8981 + 24838 + 7685)-1)*100, 1)`%). Ez fontos abból a szempontból, hogy cáfolja azt a vélekedést, miszerint a gyors regisztrálás "boldog-boldogtalant" koronavírusos halottnak sorol, szemben a precíz besorolással. Sajnos, mint volt is róla szó, a precíz, HVB szerinti adatokat mindig csak a következő évben tudjuk meg (és nem is az elején).
...
Ami az említett különbségeket illeti, vegyük példának Szlovákiát és Lettországot. Szlovákia szűk háromszor nagyobb ország lélekszámban (5,5 és 1,9 millió fő) és szűk háromszor annyi az abszolút többlethalálozása is (`r format(round(res[age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&date=="2023-06-26"&geo=="SK"]$cumexcess, -2), scientific = FALSE)` és `r format(round(res[age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&date=="2023-06-26"&geo=="LV"]$cumexcess, -2), scientific = FALSE)`). Ezért kerültek szinte pontosan egymás fölé: a lélekszámra vetített többlethalálozásaik nagyon pontosan egyeznek. Igen ám, de Lettországban sokkal nagyobb a várt halandóság! A járvány időszaka alatt kumuláltan `r format(round(res[age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&date=="2023-06-26"&geo=="LV"]$cumexpected, -2), scientific = FALSE)` fő, míg Szlovákiában `r format(round(res[age=="TOTAL"&sens==FALSE&ED=="ExPost"&model=="quasipoisson"&date=="2023-06-26"&geo=="SK"]$cumexpected, -2), scientific = FALSE)` fő (ne felejtsük el, hogy Szlovákia majdnem háromszor akkora lélekszámmal bír). Ez az alapján sem meglepő, hogy Lettországban egyszerűen nagyobb a nyers halandóság, például a koronavírus-járványt megelőző 5 évben `r round(sum(RawData[geo=="LV"&year>=2015&year<=2019]$outcome)/sum(RawData[geo=="LV"&year>=2015&year<=2019]$population)*1000*52, 1)`/1000 fő/év, míg Szlovákiában csak `r round(sum(RawData[geo=="SK"&year>=2015&year<=2019]$outcome)/sum(RawData[geo=="SK"&year>=2015&year<=2019]$population)*1000*52, 1)`/1000 fő/év. Azaz hiába halnak meg ugyanannyian lakosságarányosan, Szlovákiában úgy egyébként jóval kevesebb halálozás történik, így ugyanaz a -- lakosságarányos -- többlet ilyen értelemben durvább helyzetet jelez. Ezt tükrözi, hogy a függőleges tengelyen Szlovákia jóval feljebb került.
...
A számítások aktualizálásának dátuma: `r Sys.Date()`. A többlethalálozást számító csomag (`excessmort`) verziószáma `r packageVersion("excessmort")`.

Elsőként betöltjük a szükséges könyvtárakat, elvégzünk pár egyéb előkészületet:

```{r konyvtar, eval = FALSE}
```

A mortalitási adatokat az Eurostat-tól kérjük le (a `demo_r_mwk_ts` [adatbázist](https://ec.europa.eu/eurostat/databrowser/view/demo_r_mwk_ts/default/table?lang=en) használva az országos, és a `demo_r_mwk3_ts` [adatbázist](https://ec.europa.eu/eurostat/databrowser/view/demo_r_mwk3_ts/default/table) használva a regionális adatokhoz), az `eurostat` csomag használatával, majd leszűrjük az adattáblákat és kikódoljuk a szükséges változókat:

```{r adatbetoltes, eval = FALSE, message = FALSE}
```
...
```{r hatterpopulacio, eval = FALSE, message = FALSE}
```
...
```{r parameterbeallitas, eval = FALSE, message = FALSE}
```
...
```{r szamitas, eval = FALSE, message = FALSE}
```

Ezután egyesítjük a többlethalálozási adatbázist a korábbi adatokkal, és leszűrjük a járvány időszakára:

```{r veglegesadatok, eval = FALSE}
```

Kiszámoljuk a -- járvány eleje óta -- kumulált többletet és kumulált várt értéket, illetve a populációból egy (kumulált) átlagot:

```{r tobbletszamolas, eval = FALSE}
```

A `geo`-t átalakítjuk faktorrá, és beállítjuk, hogy Magyarország az utolsó legyen, hogy az ábrázolásnál az kerüljön a legtetejére:

```{r geofaktor, eval = FALSE}
```

A térkép-adatokkal is összekapcsoljuk az eredményeket a későbbi térképes ábrázolás érdekében, illetve kibővítjük azokat az országok és megyék elnevezéseivel, hogy ne csak kódjaink legyenek:

```{r terkepelokeszites, eval = FALSE, message = FALSE}
```

Következő lépésben kikódoljuk a 3 betűs ISO-országkódot a 2 betűs Eurostat kódból, hogy később az jelentett halálozási (WHO) adatbázissal lehessen egyesíteni:

```{r isokodok, eval = FALSE}
```

A jelentett halálozással való egybevetés céljából letöltjük a WHO [adatbázisát](https://data.who.int/dashboards/covid19/data), mely tartalmazza a jelentett halálozás adatait, majd kiszámoljuk ott is az évet és a hetet:

```{r epidata, eval = FALSE}
```

Ezt egyesítjük az előbbi táblával, és a jelentett halálozásokat itt is felkumuláljuk:

```{r epimerge, eval = FALSE}
```

Végezetül pedig, most hogy minden adatfeldolgozás végére értünk, az eredményeket kimentjük, hogy más is kényelmesen fel tudja használni:
...
- [ ] A járvány végének ("fokozatos visszatérés az alapvonalhoz") jobb kezelése.
- [ ] Spline megválasztásánál vizsgálni az ex ante vs. ex post szempontot.
- [ ] A spline diagnosztikai ábráján a tört évek jobb kezelése (szezonalitás szerinti szétosztás).
...
- Tamás Ferenci. "Comparing methods to predict baseline mortality for excess mortality calculations." 
BMC Med Res Methodol. 2023 Oct 18;23(1):239. DOI: 10.1186/s12874-023-02061-w. [Link](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/s12874-023-02061-w).